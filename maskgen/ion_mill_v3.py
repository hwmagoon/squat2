...
def oxide_ion_mill_from_lhs_pt(loc, qpad_x, qpad_y, qpad_angle, oxide_overlap, N_island, len_island, width_island, sep_island,  fillet_radius,
    ...
    probe_to_island = gdspy.Rectangle(
                ...
                **pad_layer)
    # second probe to island for 4 pt measurements
    qpad2b  = gdspy.Round(
                center=(loc[0], loc[1]),
                radius=(qpad_x, qpad_y),
                initial_angle = math.pi-qpad_angle/2,
                final_angle = math.pi+qpad_angle/2,
                **pad_layer)
    qpad2b.fillet(fillet_radius)

    # probe to island
    probe_to_islandb = gdspy.Rectangle(
                (loc[0]-qpad_x*0.5, loc[1]-qpad_x*.6),
                (loc[0]+qpad_x*0.5, loc[1]+qpad_x*.6),
                **pad_layer)
    # combine probe to island with qpad2
    qpad2 = gdspy.boolean(qpad2, probe_to_island, 'or', **pad_layer)
    qpad2 = gdspy.boolean(qpad2, qpad2b, 'or', **pad_layer)
    qpad2 = gdspy.boolean(qpad2, probe_to_islandb, 'or', **pad_layer)
    ...
    probe_to_island2b = gdspy.Rectangle(
                (loc[0]+qpad_separation-sep_island+qpad_x*1.5, loc[1]-qpad_x*.6),
                (loc[0]+qpad_separation-sep_island+qpad_x*2.5, loc[1]+qpad_x*.6),
                **pad_layer)

    # second probe pad
    qpad1b  = gdspy.Round(
                center=(loc[0]+qpad_x+qpad_separation+qpad_x, loc[1]), #(l1[0]-line_xoffset-qpad_x-qpad_gap, qubit_y),
                radius=(qpad_x, qpad_y),
                initial_angle = qpad_angle/2,
                final_angle = -qpad_angle/2,
                **pad_layer)
    ...
    probe_to_island2 = gdspy.Rectangle(
                (loc[0]+qpad_x+qpad_separation-sep_island-oxide_overlap, loc[1]-oxide_overlap/2),
                (loc[0]+qpad_x+qpad_separation+oxide_overlap, loc[1]+oxide_overlap/2),
                **pad_layer)

    # combine probe to island with qpad1    
    qpad1 = gdspy.boolean(qpad1, probe_to_island2, 'or', **pad_layer)
    qpad1 = gdspy.boolean(qpad1, qpad1b, 'or', **pad_layer)
    qpad1 = gdspy.boolean(qpad1, probe_to_island2b, 'or', **pad_layer)
...
def oxide_ion_mill_refactor(loc, qpad_x, qpad_y, qpad_angle, oxide_overlap, N_depth, N_width, len_island, width_island, sep_island,  fillet_radius,
                         pad_layer, oxide_layer):
    
    # new parameters: N_depth = 1,2
    # N_width = 1,2
    # N_island = 4*N_depth*N_width 
    # N_depth = 3
    # N_width = 3
    N_island = 4*N_depth*N_width + 3
    ...
    island = gdspy.Rectangle(
            (loc[0]+qpad_x+sep_island, loc[1]-2.5*len_island),
            (loc[0]+qpad_x+sep_island+len_island, loc[1]+len_island/2),
            **oxide_layer)

    islands_list.append(island)
    
    between_island = gdspy.Rectangle(   
            (loc[0]+qpad_x+sep_island+len_island/2-oxide_overlap/2, loc[1]-2.5*len_island+oxide_overlap),
            (loc[0]+qpad_x+sep_island+len_island/2+oxide_overlap/2, loc[1]-2.5*len_island-oxide_overlap-sep_island),
            **pad_layer)
    
    between_islands_list.append(between_island)

    island = gdspy.Rectangle(
            (loc[0]+qpad_x+sep_island, loc[1]-2.5*len_island-sep_island),
            (loc[0]+qpad_x+sep_island+len_island, loc[1]-5.5*len_island-sep_island),
            **oxide_layer)
    ...
    for n in range(N_depth):
        for m in range(N_width):
            between_island = gdspy.Rectangle(
                (loc[0]+qpad_x+sep_island+oxide_overlap-m*(3*len_island+sep_island), loc[1]-5.5*len_island-sep_island+len_island/2-oxide_overlap/2-n*4*len_island),
                (loc[0]+qpad_x-oxide_overlap-m*(3*len_island+sep_island), loc[1]-5.5*len_island-sep_island+len_island/2+oxide_overlap/2-n*4*len_island),
                **pad_layer)
            ...
            island = gdspy.Rectangle(
                (loc[0]+qpad_x-m*(3*len_island+sep_island), loc[1]-4.5*len_island-sep_island-n*4*len_island),
                (loc[0]+qpad_x-3*len_island-m*(3*len_island+sep_island), loc[1]-4.5*len_island-sep_island-len_island-n*4*len_island),
                **oxide_layer)

            islands_list.append(island)

        island = gdspy.Rectangle(
            (loc[0]+qpad_x-(N_width-1)*(3*len_island+sep_island), loc[1]-4.5*len_island-sep_island-n*4*len_island),
            (loc[0]+qpad_x-len_island-(N_width-1)*(3*len_island+sep_island), loc[1]-7.5*len_island-sep_island-n*4*len_island),
            **oxide_layer)

        islands_list.append(island)

        for m in range(N_width-1,-1,-1):
            between_island = gdspy.Rectangle(
                (loc[0]+qpad_x+sep_island+oxide_overlap-m*(3*len_island+sep_island), loc[1]-7.5*len_island-sep_island+len_island/2-oxide_overlap/2-n*4*len_island),
                (loc[0]+qpad_x-oxide_overlap-m*(3*len_island+sep_island), loc[1]-7.5*len_island-sep_island+len_island/2+oxide_overlap/2-n*4*len_island),
                **pad_layer)
            ...
            island = gdspy.Rectangle(
                (loc[0]+qpad_x-(m-1)*(3*len_island+sep_island), loc[1]-6.5*len_island-sep_island-n*4*len_island),
                (loc[0]+qpad_x-3*len_island-(m-1)*(3*len_island+sep_island), loc[1]-6.5*len_island-sep_island-len_island-n*4*len_island),
                **oxide_layer)
        ...
        island = gdspy.Rectangle(
                (loc[0]+qpad_x+sep_island, loc[1]-6.5*len_island-sep_island-n*4*len_island),
                (loc[0]+qpad_x+sep_island+len_island, loc[1]-9.5*len_island-sep_island-n*4*len_island),
                **oxide_layer)
        
        islands_list.append(island)
    
    # duplicate all islands, flipped along x = loc[0]+qpad_separation/2
    reflected_islands = []
    for island in islands_list:
        island = gdspy.copy(island)
        island_new = island.mirror(p1=(loc[0]+qpad_x+qpad_separation/2, loc[1]), p2=(loc[0]+qpad_x+qpad_separation/2, loc[1]+1))
        reflected_islands.append(island_new)
    islands_list += reflected_islands

    # duplicate all between_islands, flipped along x = loc[0]+qpad_separation
    reflected_between_islands = []
    for between_island in between_islands_list:
        between_island = gdspy.copy(between_island)
        between_island_new = between_island.mirror(p1=(loc[0]+qpad_x+qpad_separation/2, loc[1]), p2=(loc[0]+qpad_x+qpad_separation/2, loc[1]+1))
        reflected_between_islands.append(between_island_new)
    between_islands_list += reflected_between_islands

    # final connection
    island = gdspy.Rectangle(
        (loc[0]+qpad_x+sep_island, loc[1]-6.5*len_island-sep_island-n*4*len_island),
        (loc[0]+qpad_x+sep_island+qpad_separation-2*sep_island, loc[1]-7.5*len_island-sep_island-n*4*len_island),
        **oxide_layer)
    ...
    text_size = 80
    # add text of even more above center of islands
    text = gdspy.Text(f"{N_island}", size=text_size, position=(loc[0]+qpad_x+(qpad_separation+sep_island+oxide_overlap-text_size)/2, loc[1]-width_island/2-100), horizontal=False, layer=oxide_layer['layer'])

    # add text of overlap above center of islands
    text2 = gdspy.Text(f"{round(oxide_overlap)}", size=text_size, position=(loc[0]+qpad_x+(qpad_separation+oxide_overlap-text_size)/2, loc[1]+width_island/2+20), layer=oxide_layer['layer'])
...
overlaps = np.linspace(4.0, 22.0, 4)
N_depths = [1,2,5]
N_widths = [1,2,5]
...
center_vals = np.array([(0.22*chip_x, 0.28*chip_y),
                        (0.62*chip_x, 0.28*chip_y),
                        (0.38*chip_x, 0.67*chip_y)])
...
for i, N_width in enumerate(N_widths):
    ## Figure out placement on chip
    chip_center = center_vals[i]

    feature_array_space = np.array([3*qpad_x+qpad_separation/2+(N_width-2)*(3.5*len_island), 2*qpad_y + N_depths[i]*(1.7*len_island)])
    ...
    for overlap in overlaps:
        for _ in range(redundancy):
            ...
            qpads, islands, between = oxide_ion_mill_refactor(loc=loc, 
                                    ...
                                    N_depth=N_depths[i], N_width=N_width, len_island=len_island, width_island=width_island,
                                    sep_island=sep_island, fillet_radius=fillet_r_small,
                                    pad_layer=layer_al_pad, oxide_layer=layer_al_oxide)
            cell_probes.add(qpads)
            cell_probes.add(between)
...
feature_array_space = (275, 0)
for i in range(redundancy*2+2):
    if i > redundancy:
        # print("second layer")
        loc = [xpos, ypos]
        ## Make junction
        ...
        text_size = 80
        # add text of overlap above center of islands
        text2 = gdspy.Text(f"2", size=text_size, position=(loc[0]+140,loc[1]+5), layer=layer_al_pad['layer'])
        cell_probes.add(text2)
        ...
        qpads, _, between = oxide_ion_mill_from_lhs_pt(loc=loc, 
                                ...
                                N_island=N, len_island=len_island, width_island=width_island,
                                sep_island=sep_island, fillet_radius=fillet_r_small,
                                pad_layer=layer_al_oxide, oxide_layer=layer_al_oxide)
        cell_islands.add(qpads)
        cell_islands.add(between)

        text_size = 80
        # add text of overlap above center of islands
        text2 = gdspy.Text(f"1", size=text_size, position=(loc[0]+140,loc[1]+5), layer=layer_al_oxide['layer'])
        cell_islands.add(text2)
